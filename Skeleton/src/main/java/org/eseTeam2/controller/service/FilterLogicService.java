...
/**
 * This class is handling the filter logic, used to Filter for specific ads. It is very development, probably going to get changed.
 * @author Icewater
 *
 */
@Service
public class FilterLogicService implements IFilterLogicService {
	...
	/**
	 * This method takes a filter from, creates an advertisement object from it,  and returns it.
	...
	public Advertisement getAdToCompare(ArrayList<String> getterMethodNames, ArrayList<String> parameterNames,
		...
		for (int i = 0; i < getterMethodNames.size(); i++) {
			...
			try {
				...
				access.setPropertyValue(parameterNames.get(i),
				c.getDeclaredMethod(getterMethodNames.get(i)).invoke(filterForm));
			} catch (Exception d) {
			}
		}
		return desiredAd;
	...
	/**
	 * this method compares a given ad to all ads of an user, and returns all ads that match the given ad.
	 * Used to Filter ads, by given parameters.
	 * @param Advertisement adToCompare
	 * @param ArrayList<String> getters
	 * @return ArrayList<Advertisement> containing all ads that match in parameters the given adToCompare
	 */
		...
		for (int i = 0; i < allAdsInDbList.size(); i++) {
			...
			for (int j = 0; j < booleanGetterNames.size(); j++) {
				...
				try {
					...
					boolean tmpDesAd = (Boolean) accessDesiredAd.getPropertyValue(booleanGetterNames.get(j)
							.substring(2));
			...
			if (!adToCompare.getWgGender().equals("dontcare")
					&& (!allAdsInDbList.get(i).getWgGender().contains(adToCompare.getWgGender())))
				stillAMatch = false;
	...
	/**
	 * This method is a smaller filter Version of the  getAdsThatMatchTheFilter method. Same functionality
	 * returns a list of all Ads which match the filter.
	 */
	...
	/**
	 * this method is used to create an advertisement from a Filter which the user has chosen. 
	 * 
	 */
	public CustomFilterAd getFilterAdToCompare(ArrayList<String> getterMethodNames, ArrayList<String> paramNames,
		...
		for (int i = 0; i < getterMethodNames.size(); i++) {
			...
			try {
				...
				access.setPropertyValue(paramNames.get(i),
						c.getDeclaredMethod(getterMethodNames.get(i)).invoke(filterForm));
			} catch (Exception d) {
			}
		}
	...
	/**
	 * this method compares 2 advertisements and returns true if the given parameters in the adToCompare match with the placedAd
	 * @param CustomFilterAd adToCompare the ad which is compared to another ad. The placeAd has to match this ad.
	 * @param Advertisement placeAd this ad gets compared to the adToCompare. 
	 * @param ArrayList<String> getters 
	 */
	public boolean isNewAdMatch(CustomFilterAd adToCompare, ArrayList<String> getters, Advertisement placedAd) {
		...
		if (!adToCompare.getWgType().contains("undef")
				&& (adToCompare.getWgType().contains(placedAd.getWgType())) == false) {

			return false;
		}

		if (!adToCompare.getWgGender().contains("dontcare")
				&& (adToCompare.getWgGender().contains(placedAd.getWgGender())) == false) {
